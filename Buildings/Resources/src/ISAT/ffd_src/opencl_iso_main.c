...
///
/// \brief  main entrance of parallel FFD program with platform and device number
...
///////////////////////////////////////////////////////////////////////////////////////
// main entrance of opencl program
// argc is the number of member in argv
// argv is the command-line argument
// Example of command line argument:
//
// Ref: https://stackoverflow.com/questions/9642732/parsing-command-line-arguments
...
int main(int argc, char *argv[])
{
  ...
  cl_kernel kernel[34] = { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
    NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL , NULL,NULL, NULL, NULL,
    NULL, NULL, NULL, NULL, NULL, NULL }; //<--- DEFINE HERE THE NUMBER OF KERNELS THAT ARE FOUND IN THE .CL FILE
  ...
  /*Translate and validate the targeted platform and device from the command-line argument*/
  parse_argument(argc, argv, platform_device);

  /*Step0: Ready to start OpenCL calculation*/
  sprintf(msg, "main(): start OpenCL FFD calculation");
  ...
  if (platform_device[0]+1>numPlatforms) {
    ...
    char *prof_info= NULL;
    size_t prof_len;
    status = clGetPlatformInfo(platforms[USE_PLATFORM], CL_PLATFORM_NAME, NULL,NULL, &prof_len);
    prof_info = (char*)malloc(sizeof(char)*prof_len);
    status = clGetPlatformInfo(platforms[USE_PLATFORM], CL_PLATFORM_NAME, prof_len, prof_info, NULL);
    if(&para.outp->version==DEBUG){
  ...
  //Wei Remeber to change the Thomas Seville back to wei_tian in the address
  //status = clBuildProgram(program, 1, &devices[0], "-I C:/Users/wei/Desktop/GPU-release/Release", NULL, NULL);
  ...
  cl_mem bindex_mobj = clCreateBuffer(context, CL_MEM_READ_WRITE, BINDEX_ROW*size*sizeof(bindex_flat), NULL, &status); //<--- BINDEX
  //cl_mem var_vx_mobj = clCreateBuffer(context, CL_MEM_READ_WRITE, size*sizeof(REAL), NULL, &status); //<--- VX
  //cl_mem var_vy_mobj = clCreateBuffer(context, CL_MEM_READ_WRITE, size*sizeof(REAL), NULL, &status); //<--- VY
  //cl_mem var_vz_mobj = clCreateBuffer(context, CL_MEM_READ_WRITE, size*sizeof(REAL), NULL, &status); //<--- VZ
  //cl_mem var_temp_mobj = clCreateBuffer(context, CL_MEM_READ_WRITE, size*sizeof(REAL), NULL, &status); //<--- T
  //cl_mem var_tmp1_mobj = clCreateBuffer(context, CL_MEM_READ_WRITE, size*sizeof(REAL), NULL, &status); //<--- temp1
  //cl_mem var_tmp2_mobj = clCreateBuffer(context, CL_MEM_READ_WRITE, size*sizeof(REAL), NULL, &status); //<--- temp2
  //cl_mem var_tmp3_mobj = clCreateBuffer(context, CL_MEM_READ_WRITE, size*sizeof(REAL), NULL, &status); //<--- temp3
  ...
  while (NEXT == 1)
  {
    ...
    /*Call back data for sequential process for tile modeling*/
    if (hasTile) {
