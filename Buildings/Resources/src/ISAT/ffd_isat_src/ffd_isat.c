...
******************************************************************************/
  ...
  //double xBoundary[nx_SIZE][2]={{25.0,30.0},{25.0,30.0}};
		double xBoundary[nx_SIZE][2] = { { 1.0,10.0 },{ 1.0,10.0 } };
  ...
  //double xBoundary2[nx_SIZE][2]={{15.0,35.0},{15.0,35.0}};
		double xBoundary2[nx_SIZE][2] = { { 1.0,10.0 },{ 1.0,10.0 } };
...
int isat_main () {
  ...
  ****************************************************************************/
  ...
  //Basic setting
  info[1] = 1;      /*info(2) if_g     = 0, build the table from scratch
  ...
  ****************************************************************************/
  ...
  //Check useISAT to protect mistaken setting (using uniform training)
  if (useTablePreparation && useISAT && !useBinarySelectedPoint)
    nDemArrEva(nx,xStep,cosim);
  ...
  if (useRoundInput){
    for (i=0; i<nx; i++){
      x[i] = my_round(x[i],digAftdec);
		...
		x[0] = cosim->ffd->input[0];
		x[1] = cosim->ffd->input[1];
...
int cosim_loop() {
	...
	while (next == 1) {
		...
		.......................................................................*/
		if (cosim->para->flag == 0) {
			/* Stop the solver*/
			next = 0;
		}

		/*.......................................................................
		| Exchange data with Modelica
		.......................................................................*/
		if (cosim->modelica->flag == 1) {

			input = read_cosim_data(cosim);
/*
			for (i_ite = 0;i_ite < nRanCall;i_ite++) {
				x[0] = input1[i_ite];
				x[1] = input2[i_ite];
