...
int isatStartCosimulation(char *cfdFilNam, char **name, double *A, double *til,
  ...
  ****************************************************************************/
  ...
  if(haveSensor) {
    ...
    cosim->isat->senVal = (double *) malloc(nSen*sizeof(double));
    if (  cosim->isat->senVal == NULL){
      ModelicaError("Failed to allocate memory for cosim->isat->senVal in cfdStartCosimulation.c");
    }
    for(i=0; i<nSen; i++) {
  ...
  cosim->isat->XiPor = (double **) malloc(nPorts*sizeof(double *));
  if (cosim->isat->XiPor == NULL){
    ModelicaError("Failed to allocate memory for cosim->isat->XiPor in cfdStartCosimulation.c");
  }
  for(i=0; i<nPorts; i++) {
    ...
    cosim->isat->XiPor[i] = NULL;
    cosim->isat->XiPor[i] = (double *) malloc(cosim->para->nXi*sizeof(double));
    if (  cosim->isat->XiPor[i] == NULL){
      ModelicaError("Failed to allocate memory for cosim->isat->XiPor[i] in cfdStartCosimulation.c");
    }
  }
  ...
  cosim->isat->CPor = (double **) malloc(nPorts*sizeof(double *));
  if (cosim->isat->CPor == NULL){
    ModelicaError("Failed to allocate memory for cosim->isat->CPor in cfdStartCosimulation.c");
  }
  for(i=0; i<nPorts; i++) {
    ...
    cosim->isat->CPor[i] = NULL;
    cosim->isat->CPor[i] = (double *) malloc(cosim->para->nC*sizeof(double));
    if (  cosim->isat->CPor[i] == NULL){
      ModelicaError("Failed to allocate memory for cosim->isat->CPor[i] in cfdStartCosimulation.c");
    }
  }

  cosim->isat->temHea = (double *) malloc(nSur*sizeof(double));
  if (cosim->isat->temHea == NULL){
    ModelicaError("Failed to allocate memory for cosim->isat->temHea in cfdStartCosimulation.c");
  }
  if(haveShade==1){
     cosim->isat->TSha = (double *) malloc(nConExtWin*sizeof(double));
     if (   cosim->isat->TSha == NULL){
       ModelicaError("Failed to allocate memory for    cosim->isat->TSha in cfdStartCosimulation.c");
     }
  }
  cosim->isat->TPor = (double *) malloc(nPorts*sizeof(double));
  if (cosim->isat->TPor == NULL){
    ModelicaError("Failed to allocate memory for cosim->isat->TPor in cfdStartCosimulation.c");
  }
